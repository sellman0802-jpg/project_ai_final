...
MQTT_PUBLISH_INTERVAL = 0.5           # เธงเธดเธเธฒเธ—เธต: เธชเนเธเธเธฅเธ—เธธเธ เน เธเธตเนเธงเธดเธเธฒเธ—เธต (เธเธฑเธเธขเธดเธเธ–เธตเนเน€เธเธดเธเนเธ)
WITHER_CLASS_ID       = 2             # id เธเธญเธเธเธฅเธฒเธช "wither" เธชเธณเธซเธฃเธฑเธเธขเธดเธ alert

# --- Detection tracking / hold bounding boxes ---
# ใช้แก้ปัญหา AI confidence ต่ำหรือ bbox กระพริบ: ถ้าเคยเห็นวัตถุแล้วหายไปชั่วคราว
# จะค้าง bbox ไว้ช่วงหนึ่ง เพื่อให้ UI และ MQTT อ่านสถานะทัน
TRACKING_ENABLED       = True
TRACK_HOLD_SECONDS     = 2.0     # ค้าง bbox หลังจาก AI ไม่ส่งมาแล้วกี่วินาที
TRACK_IOU_THRESHOLD    = 0.25    # bbox ใหม่ต้องทับกับ track เดิมอย่างน้อยเท่านี้ถึงถือว่าเป็นอันเดียวกัน
TRACK_MIN_RAW_SCORE    = 0.05    # รับ detection ดิบต่ำสุดเท่าไรเข้า tracker
TRACK_MAX_CENTER_SHIFT = 0.45    # กัน match ผิด: จุดกลางขยับได้ไม่เกิน 45% ของขนาด bbox เดิม
...
# ============================================================================
def _bbox_iou(a, b):
    """คำนวณ IoU ระหว่าง bbox 2 กล่อง: dict ที่มี xmin,ymin,xmax,ymax"""
...
def _center_distance_ratio(track_box, det_box):
    """ระยะจุดกลาง หารด้วยขนาดเฉลี่ยของ bbox เดิม ใช้กัน match ข้ามวัตถุ"""
...
class DetectionTracker:
    """
    Tracker แบบเบา ๆ สำหรับค้าง bbox ชั่วคราว
    - match ด้วย class id + IoU + center distance
    - ถ้า frame ใหม่ไม่มี bbox จะยังส่ง bbox เก่าออกไปจนเกิน TRACK_HOLD_SECONDS
    - ช่วยให้ MQTT ไม่พลาดตอน bbox กระพริบหรือ confidence ตก
    """
    ...
    def update(self, detections):
        ...
        # กรอง detection ดิบที่ต่ำมาก ๆ ทิ้งก่อนเข้า tracker
        ...
        # 1) match detection ใหม่เข้ากับ track เดิม
        for det_index, det in enumerate(detections):
            ...
            if best_track_index is not None:
                ...
                # smoothing bbox ลดการสั่นของกรอบเล็กน้อย
                alpha = 0.65
                for key in ("xmin", "ymin", "xmax", "ymax"):
                    track[key] = int(alpha * det[key] + (1.0 - alpha) * track[key])

                # ใช้คะแนนล่าสุด แต่เก็บ score สูงสุดเล็กน้อยเพื่อไม่ให้หลุดง่ายเกินไป
        ...
        # 2) detection ใหม่ที่ไม่ match ใคร สร้าง track ใหม่
        for det_index, det in enumerate(detections):
        ...
        # 3) track ที่ไม่ได้ถูก update ให้ mark ว่าเป็น held
        for track_index, track in enumerate(self.tracks):
            if track_index not in matched_track_indexes:
                # ถ้าเป็น track ที่เพิ่งสร้างในรอบนี้ ไม่ต้องถือว่า missed
                if track.get("last_seen", now) < now:
                    track["missed"] = int(track.get("missed", 0)) + 1
                    track["held"] = True

        # 4) ลบ track ที่หายเกินเวลาหน่วง
        alive = []
        ...
        # 5) คืนค่าออกไปเป็น detection list สำหรับ UI/MQTT
        output = []
        for track in self.tracks:
            out = {
                "xmin": int(track["xmin"]),
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # หน่วง/track bounding box เพื่อกัน bbox กระพริบและช่วยให้ MQTT อ่านทัน
    ...
    def _toggle_connection(self):
        if self.worker and self.worker.isRunning():
            ...
            # เริ่มรอบใหม่ให้ล้าง track เก่าออกก่อน
            self.tracker.reset()
    ...
    def _on_frame(self, frame, detections):
        # ใช้ tracker ค้าง bbox ที่เคยเห็นไว้ชั่วคราว เพื่อกัน bbox กระพริบ/หลุด
        # raw detections = ผลจาก Coral ในเฟรมนั้น
        # tracked_detections = ผลหลัง match + hold สำหรับแสดงผลและส่ง MQTT
        if TRACKING_ENABLED:
        ...
        # ส่งขึ้น MQTT แบบ throttle โดยใช้ bbox ที่ผ่าน tracker แล้ว
        now = time.time()
